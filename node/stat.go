...
type ChannelStat struct {
	...
	Remove uint64 // total remove count
}
...
func (s *ChannelStat) Stat() []byte {
	...
	res["create"] = s.Create
	res["remove"] = s.Remove
	res["current"] = hub.channelList.Count()
	return jsonRes(res)
}
...
func StartStat() {
	...
	log.Info("Start stat listen addr: %s", Conf.StatBind)
	go statListen(Conf.StatBind)
}
...
func ChInfoStat(key string) []byte {
	res := map[string]interface{}{}
	if channel := hub.channelList.Get(key); channel != nil {
		res["channel"] = map[string]interface{}{"len": channel.Len()}
	} else {
		return nil
	}
	return jsonRes(res)
